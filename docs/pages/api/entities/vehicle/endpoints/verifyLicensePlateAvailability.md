...
Se devolverá `true` si la patente del vehículo ya está en uso y el `id` del vehículo con esa patente no coincide con el `id` pasado por parámetro. De lo contrario, se devolverá `false`.
...
```
GET /api/vehicles/license-plate-exists/RR872CU/-1
...
- exists: boolean. Indica si la patente del vehículo se encuentra en uso.
